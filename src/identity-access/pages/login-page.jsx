...
function LoginPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      navigate('/dashboard')
    } catch (e) { setError(e.response?.data?.message || 'No se pudo iniciar sesion.') }
  ...
  return <main className="grid min-h-screen bg-gray-100 lg:grid-cols-2">
    <section className="flex items-center justify-center px-6 py-12"><div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-xl">
      ...
      <h1 className="mt-3 text-3xl font-bold text-gray-900">{challengeToken ? 'Verificacion 2FA' : 'Acceso operativo'}</h1>
      <p className="mt-2 text-sm text-gray-500">{challengeToken ? 'Ingresa el codigo de seis digitos de tu autenticador.' : 'Control seguro de ventas, inventario y auditoria.'}</p>
      <form className="mt-8 space-y-5" onSubmit={submit}>
        {!challengeToken ? <><label className="block text-sm font-semibold text-gray-700">Correo<input className="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-500" type="email" required value={form.correo} onChange={e=>setForm({...form,correo:e.target.value})}/></label><label className="block text-sm font-semibold text-gray-700">Password<input className="mt-2 w-full rounded-lg border border-gray-300 px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-500" type="password" required value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/></label></> : <input className="w-full rounded-lg border border-gray-300 px-4 py-4 text-center text-2xl font-bold tracking-[0.35em]" inputMode="numeric" maxLength="6" required value={form.codigo} onChange={e=>setForm({...form,codigo:e.target.value.replace(/\D/g,'')})}/>} 
        {error&&<div className="rounded-lg bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</div>}
        <button className="w-full rounded-lg bg-blue-600 py-3 font-semibold text-white hover:bg-blue-700 disabled:opacity-60" disabled={loading}>{loading?'Validando...':challengeToken?'Verificar codigo':'Iniciar sesion'}</button>
      </form>
    </div></section>
    <aside className="hidden items-center bg-slate-900 p-14 text-white lg:flex"><div className="max-w-xl"><p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-300">DataNova Solutions</p><h2 className="mt-6 text-5xl font-bold leading-tight">Inventario confiable. Operaciones auditables.</h2><p className="mt-6 text-lg leading-8 text-slate-300">Deteccion temprana de riesgos, trazabilidad estricta y decisiones basadas en datos para MYPES.</p><div className="mt-10 grid grid-cols-3 gap-4">{['2FA seguro','Stock atomico','Auditoria real'].map(x=><div key={x} className="rounded-lg border border-slate-700 bg-slate-800 p-4 text-sm font-semibold">{x}</div>)}</div></div></aside>
  </main>
}
